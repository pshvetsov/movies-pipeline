...
from src import extract_and_clean
from src import upload_to_gcs
from src import DataConfig

logging.basicConfig(level=logging.INFO, filename='webscraper.log', filemode='w', 
...
with DAG(
    ...
    pull_dataset_task = BashOperator(
        ...
        bash_command=f"kaggle datasets download -d rounakbanik/the-movies-dataset --path {download_path} --unzip"
    )
    ...
    install_dbt_task = BashOperator(
        task_id='install_dbt',
        bash_command="pip install dbt-bigquery"
    )

    run_dbt_task = BashOperator(
        task_id='run_dbt',
        bash_command="dbt run -t dev",
        cwd=DBT_WORKDIR
    )

    pull_dataset_task >> extract_and_clean_task >> upload_to_gcs_task \
    >> create_external_tables_task >> create_table_partitions_task \
    >> clean_up_task >> install_dbt_task >> run_dbt_task
