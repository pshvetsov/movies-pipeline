...
def upload_to_gcs(storage_path):
    ...
    try:
        ...
        logger.info(
            f"Start copying parquet files from {storage_path} to {gcs_path} on GCS"
        )
        pfs.copy_files(
            source=storage_path, destination=gcs_path, destination_filesystem=gcs
        )
    except Exception as e:
        # Log any exception that occurs during the process
        logger.error(
            f"An error occurred while uploading files to GCS: {e}", exc_info=True
        )
        raise
