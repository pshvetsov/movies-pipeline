...
def extract_and_clean(download_path, pqt_path):
    ...
    for file in Data.FILE_DATA:
        ...
        try:
            ...
            logger.error(
                f"File {full_path} could not be read. Error: {e}", exc_info=True
            )
        ...
        try:
            ...
            if "no_zero_columns" in file:
                try:
                    ...
                    logger.warning(
                        f"Error during removing zero values. Error: {e}",
                        exc_info=True,
                    )
                ...
                if "production_countries" in col:
                    ...
                    try:
                        df["production_countries"] = (
                            ...
                            )  # Expand rows (since single dict - get rid of list)
                        ...
                        logger.warning(
                            f"Parsing production countries failed. Error: {e}",
                            exc_info=True,
                        )
            ...
            logger.error(
                f"An error occurred while processing {file['name']}. Error: {e}",
                exc_info=True,
            )
